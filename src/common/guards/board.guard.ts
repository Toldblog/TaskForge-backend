...
export class BoardGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (request.method === 'GET') {
      ...
      else throw new ForbiddenException('The board is only accessed by its members');
    } else if (
      ...
      if (boardMember) throw new BadRequestException('You are already a member of this board');
      if (!board.visibility) {
        throw new ForbiddenException('This board is private');
      }
    } else {
      if (boardMember) return true;
      else throw new BadRequestException('You are not a member of this board');
    }
